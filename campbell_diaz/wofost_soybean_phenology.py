...
sys.path.append(r"C:\Users\gaso001\Chapter2_Soybean_NE\Campbell_NE\campbell_diaz")
...
class TemperatureReductionFactor_Rep(SimulationObject):
    ...
    def initialize(self, day, kiosk, parvalues):
        self.params = self.Parameters(parvalues)
        p = self.params
        self.alpha = log(2.)/(log((p.Tmax_Rep-p.Tmin_Rep)/(p.Topt_Rep-p.Tmin_Rep)))
        self._cache = {}
    ...
    def __call__(self, x, _cache={}):
        ...
        p = self.params
        if x < p.Tmin_Rep or x > p.Tmax_Rep:
            v = _cache[x] = 0.
        else:
            p1 = (2*(x - p.Tmin_Rep)**self.alpha)
            p2 = ((p.Topt_Rep - p.Tmin_Rep)**self.alpha)
            p3 = ((x - p.Tmin_Rep)**(2*self.alpha))
            p4 = ((p.Topt_Rep - p.Tmin_Rep)**(2*self.alpha))
            v = _cache[x] = (p1 * p2 - p3)/p4
...
class PhotoperiodReductionFactor(SimulationObject):
    ...
    def initialize(self, day, kiosk, parvalues):
        ...
        # equation based on Setiyono et al. doi 10.1016/j.fcr.2006.07.011 (Fig.12)
        # http://digitalcommons.unl.edu/agronomyfacpub/112
        ...
        if "Popt" not in parvalues:
            Popt = 10.6595 + (2.9706/MG)
            parvalues._cropdata.update({"Popt":Popt})
                                           
        if "Pcrt" not in parvalues:
            Pcrt = 16.1257 + (4.3143/MG)
            parvalues._cropdata.update({"Pcrt":Pcrt})
...
class SoybeanPhenology(SimulationObject):
    ...
    **Signals sent or handled**
    ...
    temperature_reduction_factor_Rep = Instance(TemperatureReductionFactor_Rep)
    ...
    class Parameters(ParamTemplate):
        ...
        DVRMAX = Float(-99.)  # Max development rate floral induction
    ...
    def initialize(self, day, kiosk, parvalues):
        ...
        #DVRMAX2 based on Setoyono (Fig 11)
        if "MG" not in parvalues:
            parvalues["MG"] = -99                 
        elif "MG" in parvalues:
             MG = parvalues["MG"]            
        ...
        if "DVRMAX" not in parvalues:
            DVRMAX  = 0.0294 + (0.1561/MG)  #Fig 12a in Setiyono 
            parvalues._cropdata.update({"DVRMAX":DVRMAX})                                         

        if "DVRMAX1" not in parvalues:
            #DVRMAX1  = (0.2551 + 0.0965*MG) / (1+ 2.1107 *MG)  #Fig 11a in Setiyono
            DVRMAX1  = -0.0061*MG + 0.0464
            parvalues._cropdata.update({"DVRMAX1":DVRMAX1})                                          

        if "DVRMAX2" not in parvalues:
            DVRMAX2  = (0.0563 + 0.0228*MG)/(1 + 1.9683*MG)  #Fig 11d in Setiyono                                                              
            parvalues._cropdata.update({"DVRMAX2":DVRMAX2})
            print("DVRMAX=",DVRMAX,"DVRMAX1=",DVRMAX1,"DVRMAX=",DVRMAX2)
        ...
        self.temperature_reduction_factor_Rep = TemperatureReductionFactor_Rep(day, kiosk, parvalues)
        self.params = self.Parameters(parvalues)
        ...
        self.states = self.StateVariables(kiosk, publish="DVS",
                                          ...
                                          DOS=DOS, DOE=DOE, DOR0=None, DOR1=None,
    ...
    def calc_rates(self, day, drv):
        ...
        r.TEMPRF_Rep = self.temperature_reduction_factor_Rep(drv.TEMP)
        ...
        if s.STAGE == "emerging":
            ...
            r.DVR = p.DVRMAX2 * r.PHOTORF * r.TEMPRF_Rep
            #print("temp2",r.TEMPRF,"photo",r.PHOTORF)
